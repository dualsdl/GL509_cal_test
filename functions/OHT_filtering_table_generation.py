...
def analysis(input: dict, params: dict) -> dict:
    ...
    output['GL_setting'] = input['GL_setting']
    output['ver_info'] = params.get('ver_info', '')
    ...
    # ------------------------------------------------------------------
    # oht_table 생성 규칙
    # - 저장 파일은 byte(0~255) 단위로 관리: LUT 값도 0~255로 clip
    # - "뒤에 붙이는" 방식이 아니라, 기존 LUT의 "마지막 N행"을 메타로 대체
    #   * serial: 13자리 고정(ASCII code, 0~255)
    #   * ver_info: 날짜정보(YYMMDD) -> [YY, MM, DD] 숫자(0~255)
    # ------------------------------------------------------------------
    lut_int = data[1:, :].astype(int)  # np.savetxt(fmt='%d')와 동일하게 truncation
    # 저장 파일은 byte(0~255) 단위로 관리하므로 값은 clip 하되, dtype은 int 유지
    lut_byte = np.clip(lut_int, 0, 500).astype(int)

    # ver_info 파싱: "260127" -> 26, 1, 27 (실패 시 0 처리)
    ver_raw = str(output.get('ver_info', '') or '').strip()
    try:
        yy = int(ver_raw[0:2])
        mm = int(ver_raw[2:4])
        dd = int(ver_raw[4:6])
        ...
        yy, mm, dd = 0, 0, 0
    yy = int(np.clip(yy, 0, 255))
    mm = int(np.clip(mm, 0, 255))
    dd = int(np.clip(dd, 0, 255))

    # serial 13자리 고정: 부족하면 공백으로 패딩, 길면 자름
    serial_str = str(output.get('GL_serial', '') or '')
    serial_str = (serial_str[:13]).ljust(13)
    serial_ascii = [ord(ch) & 0xFF for ch in serial_str]  # 0~255

    # 메타 저장 규칙(중요):
    # - 0열(첫 번째 컬럼)은 LUT 원본 그대로 유지(절대 변경하지 않음)
    # - 메타는 1열(두 번째 컬럼)의 "마지막 구간"에만 기록
    #
    # 1열 메타 레이아웃(각 값은 0~255 범위):
    #   col1[-20] = 0        (magic 0)
    #   col1[-19] = 0        (magic 0)
    #   col1[-18] = YY
    #   col1[-17] = MM
    #   col1[-16] = DD
    #   col1[-15] = 0        (delimiter 0)
    #   col1[-14] = 0        (delimiter 0)  <-- ver_info 와 serial 사이 구분자
    #   col1[-13:]            serial ASCII 13개(1자리=문자 1개)
    meta_seq = (
        [0, 0, yy, mm, dd, 0, 0]
        + [int(x) for x in serial_ascii]  # 13개
    )
    meta_len = len(meta_seq)  # 20

    # 0열은 건드리지 않고, 1열만 마지막 meta_len 행을 메타로 덮어씀
    lut_byte[-meta_len:, 1] = np.array(meta_seq, dtype=int)
    output['oht_table'] = lut_byte  # int 유지
