...
default_parameters = {
    ...
    "test_cond": {
        'sineva_low_reflectance': {
            'target_angle': 90.0,
            'measurements': {
                '-130.0': {'dist': [5000.0, 10.0], 'moving_speed': 100.0, 'selected_area': 1},
                '-90.0': {'dist': [5000.0, 10.0], 'moving_speed': 100.0, 'selected_area': 1},
                '-45.0': {'dist': [5000.0, 10.0], 'moving_speed': 100.0, 'selected_area': 1},
                '0.0': {'dist': [5000.0, 10.0], 'moving_speed': 100.0, 'selected_area': 1},
                '45.0': {'dist': [5000.0, 10.0], 'moving_speed': 100.0, 'selected_area': 1},
                '90.0': {'dist': [5000.0, 10.0], 'moving_speed': 100.0, 'selected_area': 1},
                '130.0': {'dist': [5000.0, 10.0], 'moving_speed': 100.0, 'selected_area': 1},
            },
    ...
    "log_path": "./log/sineva_low_reflectance_lut_gen",
}
# #############  parameter end ###############
...
def data_acquisition(input: dict, parameters: dict) -> dict:
    ...
    for target_name, target_cond in parameters["test_cond"].items():
        ...
        for device_angle, params in target_cond['measurements'].items():            
            ...
            # 초기 위치로 이동
            stage_etel.moveTo(py_stage_etel.STAGE_AXIS_INDEX.LINEAR_TARGET, 5000, 400.0)
...
def analysis(input: dict, parameters: dict) -> dict:    
    ...
    sineva_low_reflectance_dist = np.array([])
    sineva_low_reflectance_intensity = np.array([])
    
    # 각도별 색상을 위한 colormap 설정
    device_angles_list = []
    for target_name in input.keys():
        if target_name == 'GL_serial' or target_name == 'test_time':
            ...
            for device_angle_str in input[target_name].keys():
                device_angles_list.append(float(device_angle_str))
    
    unique_angles = sorted(set(device_angles_list))
    if len(unique_angles) <= 10:
        cmap = plt.colormaps['tab10']
    else:
        cmap = plt.colormaps['tab20']
    angle_color_map = {angle: cmap(i / len(unique_angles)) for i, angle in enumerate(unique_angles)}
    ...
    for target_name in input.keys():
        if target_name == 'GL_serial' or target_name == 'test_time':
            ...
            for device_angle_str in input[target_name].keys():
                ...
                # 스캔 각도별로 다른 색상과 레이블로 scatter 플롯
                angle_color = angle_color_map[device_angle]
                plt.scatter(ROI_dist, ROI_intensity,
                            c=[angle_color],
                            label=f'Angle {device_angle_str}°',
                            alpha=0.3,)

                sineva_low_reflectance_dist = np.hstack((sineva_low_reflectance_dist, ROI_dist))
                sineva_low_reflectance_intensity = np.hstack((sineva_low_reflectance_intensity, ROI_intensity))

    dist_unique, _ = np.unique(sineva_low_reflectance_dist, return_index=True)
    ...
    for i, ux in enumerate(d_table):
        ...
        if mask.size > 0:
            ...
            filtered = sineva_low_reflectance_intensity[sineva_low_reflectance_dist == current_dist]
            if filtered.size > 0:
    ...
    plt.title('Sineva Low Reflectance LUT Generation')
...
def run(input: dict, parameters: dict) -> dict:
    ...
    np.savetxt(
        f"{default_parameters['log_path']}/{raw_data['GL_serial']}_{raw_data['test_time']}_sineva_low_reflectance_filtering_table.csv",
        result['data'],
        delimiter=",",
        comments='',
        fmt='%d'
