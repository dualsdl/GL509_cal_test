...
def main(page: ft.Page):
    page.title = "GL-509 1st_cal_and_test"
    ...
    OHT_ld_high_voltage_field = ft.TextField(label="OHT LD High Voltage (0.1V)", width=600, value=str(loaded_walk_error['OHT_ld_high_voltage']))
    OHT_pd_high_voltage_field = ft.TextField(label="OHT PD High Voltage (V)", width=600, value=str(loaded_walk_error['OHT_pd_high_voltage']))
    OHT_BR_intensity_field = ft.TextField(label="OHT BR Intensity (0.1ns)", width=600, value=str(loaded_walk_error['OHT_BR_intensity']))
    OBS_ld_high_voltage_field = ft.TextField(label="OBS LD High Voltage (0.1V)", width=600, value=str(loaded_walk_error['OBS_ld_high_voltage']))
    OBS_pd_high_voltage_field = ft.TextField(label="OBS PD High Voltage (V)", width=600, value=str(loaded_walk_error['OBS_pd_high_voltage']))
    OBS_BR_intensity_field = ft.TextField(label="OBS BR Intensity (0.1ns)", width=600, value=str(loaded_walk_error['OBS_BR_intensity']))

    # 수평도 검사 설정 필드 추가
    tx_level_ld_hv_field = ft.TextField(label="LD High Voltage", width=600, value=str(loaded_tx_level['LD_HV']))
    ...
    # 복잡한 데이터 구조 처리 (리스트와 딕셔너리를 포함하는 구조)
    distance_performance_oht_cal_dist_points_field = ft.TextField(label="OHT 계산 거리 포인트", width=600, height=600, multiline=True, value=json.dumps(loaded_distance_performance['oht_cal_dist_points'], indent=4))
    distance_performance_obs_cal_dist_points_field = ft.TextField(label="OBS 계산 거리 포인트", width=600, height=600, multiline=True, value=json.dumps(loaded_distance_performance['obs_cal_dist_points'], indent=4))
    distance_performance_oht_test_dist_points_field = ft.TextField(label="OHT 테스트 거리 포인트", width=600, height=600, multiline=True, value=json.dumps(loaded_distance_performance['oht_test_dist_points'], indent=4))
    distance_performance_obs_test_dist_points_field = ft.TextField(label="OBS 테스트 거리 포인트", width=600, height=600, multiline=True, value=json.dumps(loaded_distance_performance['obs_test_dist_points'], indent=4))
    ...
    distance_performance_roi_width_field = ft.TextField(label="ROI 너비", width=600, value=str(loaded_distance_performance['ROI_width']))
    distance_performance_precision_pass_criteria_field = ft.TextField(label="정확도 합격 기준", width=600, value=str(loaded_distance_performance['precision_pass_criteria']))
    distance_performance_accuracy_pass_criteria_field = ft.TextField(label="정확도 합격 기준", width=600, value=str(loaded_distance_performance['accuracy_pass_criteria']))

    # OHT 필터링 테이블 생성 설정 필드 추가
    OHT_filtering_table_generation_roi_width_field = ft.TextField(label="ROI 너비", width=600, value=str(loaded_OHT_filtering_table_generation['ROI_width']))
    ...
    max_distance_roi_width_field = ft.TextField(label="ROI 너비", width=600, value=str(loaded_max_distance['ROI_width']))
    ...
    def save_settings_to_json(e=None):
        data = {
            ...
            'home_position': {
                ...
                'home_position_pass_criteria': int(home_position_pass_criteria_field.value)
            },
            'distance_performance': {
                'OHT_target_angle': float(loaded_distance_performance['OHT_target_angle']),
                'OBS_target_angle': float(loaded_distance_performance['OBS_target_angle']),
                ...
                'oht_cal_dist_points': parse_dict_from_string(loaded_distance_performance['oht_cal_dist_points']),
                'obs_cal_dist_points': parse_dict_from_string(loaded_distance_performance['obs_cal_dist_points']),
                'oht_test_dist_points': parse_dict_from_string(loaded_distance_performance['oht_test_dist_points']),
                'obs_test_dist_points': parse_dict_from_string(loaded_distance_performance['obs_test_dist_points']),
                'logging_frame_num': int(loaded_distance_performance['logging_frame_num']),
                'ROI_width': float(loaded_distance_performance['ROI_width']),
                'precision_pass_criteria': float(loaded_distance_performance['precision_pass_criteria']),
                'accuracy_pass_criteria': float(loaded_distance_performance['accuracy_pass_criteria'])
            },
        ...
        loaded_distance_performance['device_rotation_speed'] = float(distance_performance_device_rotation_speed_field.value)
        loaded_distance_performance['LINEAR_STAGE_OFFSET'] = float(distance_performance_linear_stage_offset_field.value)
    ...
    def on_test_type_change(e):
        ...
        if selected == "1차 보정검사(공통)":
            inspection_type_dropdown.options = [
                ...
                ft.dropdown.Option("Walk Error LUT Generation")
            ]
    ...
    test_functions = {
        ...
        "Walk Error LUT Generation": walk_error_lut_test,
    ...
    def start_inspection(e):
        ...
        if inspection_type == "빔 사이즈 측정":
            run_beam_size_test()
        elif inspection_type == "Walk Error LUT Generation":
    ...
    first_cal_common_container = ft.Column([
        ...
        ft.Text("Walk Error LUT Generation", size=16, weight=ft.FontWeight.BOLD),
        ft.Row([
            walk_error_field,
            ft.ElevatedButton(
                "파일 열기",
    ...
    settings_tab = ft.Tab(
        ...
        content=ft.Container(
            content=ft.Column([
                ...
                ft.ExpansionTile(
                    title=ft.Text("Walk Error LUT Settings", size=16, weight=ft.FontWeight.BOLD),
                    controls=[
                        ft.Container(
                            content=ft.Column([
                ...
                ft.ExpansionTile(
                    title=ft.Text("홈 포지션 설정", size=16, weight=ft.FontWeight.BOLD),
                    controls=[
                        ft.Container(
                            content=ft.Column([
                ...
                ft.ExpansionTile(
                    title=ft.Text("거리 성능 설정", size=16, weight=ft.FontWeight.BOLD),
                    controls=[
                        ft.Container(
                            content=ft.Column([
                ...
                ft.ExpansionTile(
                    title=ft.Text("OHT 필터링 테이블 생성 설정", size=16, weight=ft.FontWeight.BOLD),
                    controls=[
                        ft.Container(
                            content=ft.Column([
                ...
                ft.ExpansionTile(
                    title=ft.Text("OHT 필터링 검증 설정", size=16, weight=ft.FontWeight.BOLD),
                    controls=[
                        ft.Container(
                            content=ft.Column([
                ...
                ft.ExpansionTile(
                    title=ft.Text("후방 커버 검사 설정", size=16, weight=ft.FontWeight.BOLD),
                    controls=[
                        ft.Container(
                            content=ft.Column([
